  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        {/* Header */}
        <div className="p-4 flex items-center h-16 border-b border-slate-100 dark:border-slate-800">
          {!collapsed ? (
        ...
        <div className="p-3 border-t border-slate-100 dark:border-slate-800">
          {!collapsed && userData && (
            <div className="mb-4 px-2">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-[10px]">Logged in as</p>
              <p className="text-sm font-medium truncate">{userData.email}</p>
              <p className="text-[10px] text-primary font-bold uppercase tracking-tight">{userData.role.replace('_', ' ')}</p>
            </div>
          )}
          <Button 
            variant="ghost" 
